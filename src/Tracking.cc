        ...
        for (auto i : mCurrentFrame.mvpMapPoints)
        {
            if (i)
            {
#ifdef CASRF
                {
                    ...
                    // } while (!atomic_compare_exchange_strong(&(i->mReferencecount_lastframe_CAS), &old_value,
                    // new_value));
                    int old_value, new_value;
                    old_value = (i)->mReferencecount_lastframe_CAS;
                    new_value = old_value + 1;
                    while (
                        !atomic_compare_exchange_strong(&((i)->mReferencecount_lastframe_CAS), &old_value, new_value))
                    {
                        new_value = old_value + 1;
                    }
                }
            ...
            if (pRep)
            {
                if (mLastFrame.mvpMapPoints[i])
                {
#ifdef CASRF
                    {
                        ...
                        // } while
                        // (!atomic_compare_exchange_strong(&(mLastFrame.mvpMapPoints[i]->mReferencecount_lastframe_CAS),
                        // &old_value, new_value));
                        int old_value, new_value;
                        old_value = mLastFrame.mvpMapPoints[i]->mReferencecount_lastframe_CAS;
                        new_value = old_value - 1;
                        while (!atomic_compare_exchange_strong(
                            &(mLastFrame.mvpMapPoints[i]->mReferencecount_lastframe_CAS), &old_value, new_value))
                        {
                            new_value = old_value - 1;
                        }
                    }
...
#ifdef CASRF
                {
                    ...
                    // } while (!atomic_compare_exchange_strong(&(pRep->mReferencecount_lastframe_CAS), &old_value,
                    // new_value));
                    int old_value, new_value;
                    old_value = (pRep)->mReferencecount_lastframe_CAS;
                    new_value = old_value + 1;
                    while (!atomic_compare_exchange_strong(&((pRep)->mReferencecount_lastframe_CAS), &old_value,
                                                           new_value))
                    {
    ...
    for (size_t j = 0; j < vDepthIdx.size(); j++)
    {
        ...
        if (bCreateNew)
        {
...
#ifdef CASRF
            {
                ...
                // } while (!atomic_compare_exchange_strong(&(pNewMP->mReferencecount_lastframe_CAS), &old_value,
                // new_value));
                int old_value, new_value;
                old_value = (pNewMP)->mReferencecount_lastframe_CAS;
                new_value = old_value + 1;
                while (
                    !atomic_compare_exchange_strong(&((pNewMP)->mReferencecount_lastframe_CAS), &old_value, new_value))
                {
                    new_value = old_value + 1;
                }
            }
        ...
        for (vector<MapPoint *>::const_iterator itMP = vpMPs.begin(), itEndMP = vpMPs.end(); itMP != itEndMP; itMP++)
        {
            ...
            if (!pMP->isBad())
            {
...
#ifdef CASRF
                {
                    ...
                    // } while (!atomic_compare_exchange_strong(&(pMP->mReferencecount_msp_CAS), &old_value,
                    // new_value));
                    int old_value, new_value;
...
void Tracking::UpdateLocalKeyFrames()
{
         ...
         it++)
    {
...
#ifdef CASRF
        {
            ...
            int old_value, new_value;
            old_value = (pKF)->mReferencecount_mob_CAS;
            new_value = old_value + 1;
            while (!atomic_compare_exchange_strong(&((pKF)->mReferencecount_mob_CAS), &old_value, new_value))
            {
                new_value = old_value + 1;
            ...
            if (!pNeighKF->isBad())
            {
                if (pNeighKF->mnTrackReferenceForFrame != mCurrentFrame.mnId)
                {
#ifdef CASRF
                    {
                        ...
                        // } while (!atomic_compare_exchange_strong(&(pNeighKF->mReferencecount_ockf_CAS), &old_value,
                        // new_value));
                        int old_value, new_value;
                        old_value = (pNeighKF)->mReferencecount_ockf_CAS;
                        new_value = old_value + 1;
                        while (!atomic_compare_exchange_strong(&((pNeighKF)->mReferencecount_ockf_CAS), &old_value,
                                                               new_value))
                        {
                    ...
                    // }
                    {
                        if ((pNeighKF)->thread_id_collection_map.find(this_thread::get_id()) !=
                    ...
                    grc_vNeighs.push_back(pNeighKF);
            ...
            if (pParent->mnTrackReferenceForFrame != mCurrentFrame.mnId)
            {
                ...
                for (auto itr : vNeighs)
                {
#ifdef CASRF
                    {
                        ...
                        // } while (!atomic_compare_exchange_strong(&(itr->mReferencecount_ockf_CAS), &old_value,
                        // new_value));
                        int old_value, new_value;
                        old_value = (itr)->mReferencecount_ockf_CAS;
                        new_value = old_value - 1;
                        while (
                            !atomic_compare_exchange_strong(&((itr)->mReferencecount_ockf_CAS), &old_value, new_value))
                        {
                            new_value = old_value - 1;
                        }
                    }
