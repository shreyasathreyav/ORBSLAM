...
namespace ORB_SLAM3
{
    ...
    MapGarbageCollector::MapGarbageCollector(Map *map, LocalMapping *local_mapping_thread) : map(map), local_mapping_thread(local_mapping_thread)
    {
    }
    ...
    void MapGarbageCollector::DeleteMPs()
    {
        ...
        {
            MapPoint *mp = *it;
            // local_mapping_thread->mlpRecentAddedMapPoints.erase(mp->GetId()); // deleteing int of mappoint from the hashmap which may not be required hence commenting 
            if (mp->IsSafeToErase())                                               // mlpRecentAddedMapPoints is a list here unlike edge
            {
                cout << mp->mnId ; 
                unique_lock<mutex> lock(map->mMutexGarbageLists);
                MPsLive->erase(mp);
