...
// }
#ifdef CASRF
                {
                    if ((*it)->mReferencecount_canonicalmp_CAS == 0 && (*it)->mReferencecount_msp_CAS == 0 &&
                        (*it)->mReferencecount_lastframe_CAS == 0)
                    {
                        totaldeletion_mp++;
                        (*it)->pass_d = true;
                        auto something = it;
...
#endif
#ifdef RF
                // Have to complete the instrumentation for this in the final stop
                {
                    // if ((*it)->mReferencecount_canonicalmp == 0 && (*it)->mReferencecount_msp == 0 &&
                    //     (*it)->mReferencecount_lastframe == 0)
                    {
                        // for (auto checker : (*it)->thread_id_collection_map)
                        if (((*it)->tracking_count + (*it)->local_mapping_count + (*it)->loop_closing_count + (*it)->viewer_count) == 0)
                        {

                            // cout << "Thread: " << key << " Value :" << value << endl;
        ...
        for (size_t i = 0; i < vpNeighKFs.size(); i++)
        {
            ...
            // }
            {
                ...
                //     vpNeighKFs[i]->thread_id_collection_map.end())
                {
                    ...
                    if (checker_thread_id == vpNeighKFs[i]->tracking_thread_id)
                    {

                        vpNeighKFs[i]->tracking_count++;
                    }
                    else if (checker_thread_id == vpNeighKFs[i]->local_mapping_thread_id)
                    {

                        vpNeighKFs[i]->local_mapping_count++;
                    }
                    else if (checker_thread_id == vpNeighKFs[i]->loop_closing_thread_id)
                    {

                        vpNeighKFs[i]->loop_closing_count++;
                    }
                    else
                    {

                        vpNeighKFs[i]->viewer_count++;
                    }
                }
                // else
                // {
            ...
            if (!mbMonocular)
            {
                if (baseline < pKF2->mb)
                {
                    ...
                    {
                        int old_value{vpNeighKFs[i]->mReferencecount_ockf_CAS}, new_value{old_value - 1};
                        while (!atomic_compare_exchange_strong(&(vpNeighKFs[i]->mReferencecount_ockf_CAS), &old_value,
                                                               new_value))
                        {
                            new_value = old_value - 1;
                        }
    ...
    void LocalMapping::SearchInNeighbors()
    {
        ...
        for (vector<KeyFrame *>::const_iterator vit = vpNeighKFs.begin(), vend = vpNeighKFs.end(); vit != vend; vit++)
        {
            ...
            // Need to add an increment here for this container
...
// /pkf
#ifdef CASRF
                        {
                            int old_value{(*vit)->mReferencecount_ockf_CAS}, new_value{old_value - 1};
                            while (
                                !atomic_compare_exchange_strong(&((*vit)->mReferencecount_ockf_CAS), &old_value, new_value))
                            {
                                new_value = old_value - 1;
                            }
                        }
...
// /pkf
        ...
        // cout <<
        // "############################################################################################################################"
        // << endl; vector<KeyFrame *> cont_del(arr.begin(), arr.end());
        for (auto it = arr.begin(); it != arr.end();)
        {
// cout << "Hello";
// if ((*it)->mReferencecount_canonical_CAS != (*it)->mReferencecount_canonical)
//     cout << "Canonical Count " << (*it)->mReferencecount_canonical << "AND CAS " <<
//     (*it)->mReferencecount_canonical_CAS << endl;
...
#ifdef CASRF
            {
                if ((*it)->mReferencecount_mob_CAS == 0 && (*it)->mReferencecount_ockf_CAS == 0 &&
                    (*it)->mReferencecount_canonical_CAS == 0)
                {
                    totaldeletion++;
                    auto something = it;
                    it = arr.erase(it);
...
#endif
#ifdef RF
            // Have to complete this for t_count
            {
                // if ((*it)->mReferencecount_mob == 0 && (*it)->mReferencecount_ockf == 0 &&
                //    (*it)->mReferencecount_canonical == 0)
                {
