...
std::map<KeyFrame *, std::tuple<int, int>> MapPoint::GetObservations(bool flag)
{
    ...
    for (auto it : mObservations)
    {
#ifdef CASRF
        {
            ...
            int old_value, new_value;
            old_value = it.first->mReferencecount_mob_CAS;
            new_value = old_value + 1;
            while (!atomic_compare_exchange_strong(&(it.first->mReferencecount_mob_CAS), &old_value, new_value))
            {
                new_value = old_value + 1;
            ...
            for (auto itx : it.first->mvpMapPoints)
            {
                ...
                if (itx != NULL && itx->mnId == this->mnId)
                {
#ifdef CASRF
                    {
                        ...
                        // } while (!atomic_compare_exchange_strong(&(itx->mReferencecount_canonicalmp_CAS), &old_value,
                        // new_value));
                        int old_value, new_value;
                        old_value = itx->mReferencecount_canonicalmp_CAS;
                        new_value = old_value - 1;
                        while (!atomic_compare_exchange_strong(&(itx->mReferencecount_canonicalmp_CAS), &old_value,
                                                               new_value))
                        {
