...
namespace ORB_SLAM3
{
    ...
    MapPoint::MapPoint() : mnFirstKFid(0), mnFirstFrame(0), nObs(0), mnTrackReferenceForFrame(0), safeToErase(true),
    ...
    MapPoint::MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map *pMap) : mnFirstKFid(pRefKF->mnId), mnFirstFrame(pRefKF->mnFrameId), nObs(0), mnTrackReferenceForFrame(0),
                                                                                  mnLastFrameSeen(0), mnBALocalForKF(0), mnFuseCandidateForKF(0), mnLoopPointForKF(0), mnCorrectedByKF(0),
                                                                                  mnCorrectedReference(0), mnBAGlobalForKF(0), mpRefKF(pRefKF), mnVisible(1), mnFound(1), safeToErase(true),
    ...
    MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map *pMap, Frame *pFrame, const int &idxF) : mnFirstKFid(-1), mnFirstFrame(pFrame->mnId), nObs(0), mnTrackReferenceForFrame(0), mnLastFrameSeen(0),
                                                                                                ...
                                                                                                mnFound(1), mpReplaced(NULL), mpMap(pMap), mnOriginMapId(pMap->GetId())
    {
        SetWorldPos(Pos);
    ...
    void MapPoint::SetEraseOK()
    {
        safeToErase = true;
    }

    void MapPoint::SetNotErase()
    {
        safeToErase = false;
    }
