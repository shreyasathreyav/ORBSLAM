                ...
                if (numProjMatches >= nProjMatches)
                {
                    ...
                    if (numOptMatches >= nSim3Inliers)
                    {
                        ...
                        if (numProjOptMatches >= nProjOptMatches)
                        {
                            ...
                            // mpCurrentKF->GetBestCovisibilityKeyFrames(nNumCovisibles, true);
                            vector<KeyFrame *> vpCurrentCovKFs =
                            ...
                            for (auto i : vpCurrentCovKFs)
                            {
                                ...
                                {
                                    int old_value{i->mReferencecount_ockf_CAS}, new_value{old_value - 1};
                                    while (!atomic_compare_exchange_strong(&(i->mReferencecount_ockf_CAS), &old_value,
                                                                           new_value))
                                    {
                                        new_value = old_value - 1;
                                    }
...
// This function was never called during testing
