...
namespace ORB_SLAM3
{
    ...
    bool LoopClosing::NewDetectCommonRegions()
    {
        ...
        if (mpTracker->mSensor == System::STEREO && mpLastMap->GetAllKeyFrames(true).size() < 5) // 12
        {
        ...
        if (mpLastMap->GetAllKeyFrames(true).size() < 12)
        {
        ...
        for (KeyFrame *pKFi : vpBowCand)
        {
            ...
            if (numBoWMatches >= nBoWMatches) // TODO pick a good threshold
            {
                ...
                if (bConverge)
                {
                    ...
                    for (auto itr : vpCovKFi)
                    {
                        unique_lock<mutex> lock(itr->mMutexreferencecount);
                        if(find(notvpCovKFi.begin(), notvpCovKFi.end(), itr) == notvpCovKFi.end())
                        {
                    ...
                    notvpCovKFi.insert(pMostBoWMatchesKF);
                    set<KeyFrame *> spCheckKFs(vpCovKFi.begin(), vpCovKFi.end());
                    for (auto itr : spCheckKFs)
                    {
                        unique_lock<mutex> lock(itr->mMutexreferencecount);
                        if(find(notvpCovKFi.begin(), notvpCovKFi.end(), itr) == notvpCovKFi.end())
                        {
    ...
    void LoopClosing::RunGlobalBundleAdjustment(Map *pActiveMap, unsigned long nLoopKF)
    {
        ...
        vnGBAKFs.push_back(pActiveMap->GetAllKeyFrames(true).size());
